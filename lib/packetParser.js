...
class PacketParser {
    ...
    switch (device) {
      case Device.TERMINAL:
        switch (protocolNumber) {
          ...
          case 0x32: return new TerminalLocation(0x32);
          case 0x33: return new TerminalLocation(0x33);
      ...
      case Device.SERVER:
        switch (protocolNumber) {
          ...
          case 0x32: return new ServerLocation(0x32);
          case 0x33: return new ServerLocation(0x33);
  ...
  static parse(data, device) {
    ...
    while (!reader.eof) {
      ...
      // Start bytes
      const startBytes = reader.readBytes(2);
  ...
  /*    const offset = 4 + packetLengthBytes;
      ...
      // Stop bytes
      const stopBytes = reader.peekBytes(2, packetLength);
      ...
      const crcEnd = packetOffset + packetLength + packetLengthBytes;
      const crcOffset = packetOffset + packetLength + packetLengthBytes;

      const encryptedCrc = (device == Device.TERMINAL) && (protocolNumber === 0x01)
      const errorCheck = reader.peekWordAt(crcOffset);
      const crc = Concox.crcRange(reader.data, crcBegin, crcEnd, encryptedCrc);
      ...
      if (packet) {
        ...
        const errorCheck = reader.readWord(); // error check

        if (errorCheck !== crc)
          throw new Error('Invalid error check code');

        const stopBit = reader.readWord(); // stop bit
